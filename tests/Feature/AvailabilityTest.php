...
class AvailabilityTest extends TestCase
{
    ...
    public function test_availability_returns_rooms_without_overlapping_bookings(): void
    {
        ...
        $response = $this->postJson('/api/v1/availability', [
            'paxes' => 2, 'checkin' => '2026-10-20', 'checkout' => '2026-10-25',
        ]);
        $response->assertOk()
            ->assertJsonCount(3)
    ...
    {
        $this->useFixture(1, [$this->booking('TEST001', '2026-10-01', '2026-10-02')]);
        $this->postJson('/api/v1/availability', $this->request())
    ...
    {
        $this->useFixture(1, [$this->booking('TEST001', '2026-10-02', '2026-10-04', 'CANCELLED')]);
        $this->postJson('/api/v1/availability', $this->request())
            ->assertOk()->assertJsonCount(1);
    }
    ...
    public function test_room_capacity_must_fit_requested_paxes(): void
    {
        ...
        $this->postJson('/api/v1/availability', [
            'paxes' => 5, 'checkin' => '2026-10-20', 'checkout' => '2026-10-25',
        ])->assertOk()->assertExactJson([]);
    }
    ...
    public function test_hotel_and_room_type_filters_limit_results(): void
    {
        ...
        $this->postJson('/api/v1/availability', [
            'hotel' => 'GRAND', 'roomType' => 'SUITE', 'paxes' => 4,
    ...
    {
        $this->useFixture(1, [$this->booking('TEST001', '2026-10-02', '2026-10-04')]);

        $this->postJson('/api/v1/availability', [
            ...$this->request(), 'checkin' => '2026-10-01', 'checkout' => '2026-10-02',
        ])->assertOk()->assertJsonCount(1);
    ...
    {
        $this->postJson('/api/v1/availability', [
            'paxes' => 2, 'checkin' => '2026-10-05', 'checkout' => '2026-10-01',
        ])->assertUnprocessable();
    }

    private function request(): array
    {
        return ['paxes' => 2, 'checkin' => '2026-10-02', 'checkout' => '2026-10-04'];
    }

    private function booking(string $locator, string $checkin, string $checkout, string $status = 'CONFIRMED'): Booking
    {
        return new Booking([
            'locator' => $locator, 'hotel' => 'TEST', 'roomType' => 'STANDARD',
            'paxes' => 2, 'checkin' => $checkin, 'checkout' => $checkout, 'status' => $status,
        ]);
    }
    ...
    private function useFixture(int $quantity, array $bookings, int $maxOccupancy = 2): void
    {
        ...
        $roomType = new RoomType(['name' => 'Standard Room', 'code' => 'STANDARD', 'maxOccupancy' => $maxOccupancy]);
        $inventory = new HotelRoomType(['quantity' => $quantity, 'price' => 100]);
